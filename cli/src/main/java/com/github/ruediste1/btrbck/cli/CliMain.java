...
public class CliMain {
    ...
    private void cmdVersion() {
        ...
        try {
            properties
                    .load(getClass().getResourceAsStream("version.properties"));
        }
        catch (IOException e) {
            throw new RuntimeException(e);
        }
        System.out.println(
                "BTRBCK version " + properties.getProperty("btrbck.version"));
    }
    ...
    private void parseCmdLine(String[] args) {
        CmdLineParser parser = new CmdLineParser(this,
                ParserProperties.defaults().withAtSyntax(false));
    ...
    private void cmdList() {
        if (arguments.size() == 1) {
            ...
            TreeMap<Integer, Snapshot> snapshots = streamService
                    .getSnapshots(stream);
            System.out.println(
                    "Snapshots in stream " + stream.name + " in repository "
                            + repo.rootDirectory.toAbsolutePath() + ":");
